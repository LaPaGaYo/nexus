#!/usr/bin/env bun
import { getRuntimeNexusAdapters } from '../adapters/registry';
import {
...
import { resolveInvocation } from '../commands/index';
import { defaultExecutionSelection } from '../runtime/execution-topology';
import { resolveRuntimeInvocation } from '../runtime/invocation';
import type { RuntimeOutputMode } from '../runtime/invocation';
import { resolveRuntimeCwd } from '../runtime/cwd';
...
try {
  ...
  if (argv[0] === 'do') {
    const { runDoCommandAsync } = await import('../commands/do');
    ...
    if (process.env.NEXUS_INTENT_LLM === '1') {
      const { autoRegisterLLMClassifier } = await import('../intent-classifier');
