...
- **[v1.1.1](docs/releases/2026-05-12-nexus-v1.1.1.md)** — Patch hotfix: release-tarball setup no longer fails on missing `.git` (build:versions resolves via `release.json` tag fallback), long-running provider invocations (`/build`, `/review`, `/qa`) stream stdout/stderr to the controller TTY with a dispatch banner so they no longer look like hangs, and a new test gates release pipeline against re-introduction of the gitless-install failure.
...
- **[v1.0.17](docs/releases/2026-04-13-nexus-v1.0.17.md)** — stale fix-cycle builds that predate `review_scope` now recover bounded review scope from the prior failing review, and governed review trace artifacts explicitly persist that scope in the dispatched adapter request.
...
- **[v1.0.35](docs/releases/2026-04-19-nexus-v1.0.35.md)** — fresh run worktrees now fetch and fast-forward to the remote default branch before governed handoff/build, so execution packets do not run against stale pre-merge baselines.
- **[v1.0.34](docs/releases/2026-04-19-nexus-v1.0.34.md)** — QA now distinguishes advisories from blocking failures, failing QA can route into a canonical fix-cycle, `/land-and-deploy` records CI re-entry state explicitly, deploy surfaces are classified as primary vs secondary, and live runtime branding is uniformly Nexus.
- **[v1.0.33](docs/releases/2026-04-18-nexus-v1.0.33.md)** — governed learnings, deploy contracts, verification matrices, retro continuity, attached evidence archiving, auto-refreshed follow-on summaries, and surfaced CCB latency diagnostics now flow through the mainline runtime.
- **[v1.0.32](docs/releases/2026-04-16-nexus-v1.0.32.md)** — design-bearing runs now carry a formal design contract through plan/build/review/qa/ship, governed review has stricter CCB request lifecycle recovery plus watchdogs, `/ship` owns PR handoff, and Nexus now exposes `nexus-ledger-doctor` plus precise closeout history diagnostics.
