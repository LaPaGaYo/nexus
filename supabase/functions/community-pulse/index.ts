// Nexus community-pulse edge function
...
Deno.serve(async () => {
  ...
  try {
    ...
    const { data: crashes } = await supabase
      .from("crash_clusters")
      .select("error_class, nexus_version, total_occurrences, identified_users")
      .limit(5);
    ...
    const { data: versionRows } = await supabase
      .from("telemetry_events")
      .select("nexus_version")
      .eq("event_type", "skill_run")
      .gte("event_timestamp", weekAgo)
      .limit(1000);

    for (const row of versionRows ?? []) {
      if (row.nexus_version) {
        versionCounts[row.nexus_version] = (versionCounts[row.nexus_version] ?? 0) + 1;
      }
    }
