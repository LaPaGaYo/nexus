...
After `/ship` returns, treat `.planning/current/ship/completion-advisor.json` as the canonical
next-step contract.
...
If the session is interactive, Always use AskUserQuestion for `/ship` completion.
