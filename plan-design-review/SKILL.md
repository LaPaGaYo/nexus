...
Read the compact principle reference before scoring:
`~/.claude/skills/nexus/design/references/plan-design-principles.md`

Apply these anchors in every pass: empty states are features, hierarchy is service,
specificity beats vibes, edge cases are user experiences, AI slop is the enemy,
responsive means intentional per viewport, accessibility is required, subtraction is
the default, and trust is earned at the pixel level.
...
Mode: `plan-design-review`
Reasoning: `model_reasoning_effort="high"`
Codex output header: `CODEX SAYS (design critique):`
