...
import type { StageStatus } from '../../../lib/nexus/contracts/types';
...
describe('readReviewAdvisories', () => {
  test('normalizes persisted advisory records and canonicalizes categories', () => {
    ...
    writeRepoJson(cwd, reviewAdvisoriesPath(), {
      ...
      categories: [null, 42, true, 'testing', 'unknown', 'security', 'testing', 'maintainability'],
    });
  ...
  test('honors an explicit empty categories array', () => {
    const cwd = makeTempRepo();
    writeRepoJson(cwd, reviewAdvisoriesPath(), {
      run_id: 'run-123',
      generated_at: '2026-01-01T00:00:00.000Z',
  ...
  test('returns null when advisory records omit required metadata', () => {
    const cwd = makeTempRepo();

    writeRepoJson(cwd, reviewAdvisoriesPath(), {
      generated_at: '2026-01-01T00:00:00.000Z',
      advisories: ['Add regression tests.'],
    });
  ...
  test('returns null for missing, malformed, or non-advisory payloads', () => {
    ...
    try {
      ...
      expect(warn).toHaveBeenCalledWith(expect.stringContaining('[nexus] Ignoring malformed JSON'));
    } finally {
      warn.mockRestore();
    }
...
describe('review advisory dispositions', () => {
  ...
  test('returns null when disposition records omit required metadata', () => {
    const cwd = makeTempRepo();
    writeRepoJson(cwd, reviewAdvisoryDispositionPath(), {
      run_id: 42,
      advisory_count: 0,
  ...
  test('rejects non-integer disposition advisory counts', () => {
    const cwd = makeTempRepo();
    writeRepoJson(cwd, reviewAdvisoryDispositionPath(), {
      run_id: 'run-123',
      advisory_count: 2.5,
  ...
  test('preserves explicitly null disposition selections', () => {
    const cwd = makeTempRepo();
    writeRepoJson(cwd, reviewAdvisoryDispositionPath(), {
      run_id: 'run-123',
      advisory_count: 2,
    ...
    expect(readReviewAdvisoryDisposition(cwd)).toMatchObject({
      run_id: 'run-123',
      advisory_count: 2,
      selected: null,
      selected_at: null,
