...
const HOST_ROOTS_MODULE = join(ROOT, 'lib', 'nexus', 'io', 'host-roots.ts');
...
describe('nexus host root contract', () => {
  test('defines Nexus primary roots with Nexus-only live metadata', async () => {
    ...
    const mod = await import('../../../lib/nexus/io/host-roots');
  ...
  test('resolves only Nexus-owned active state roots', async () => {
    ...
    const existingRoot = mod.resolveHostStateRoot({
      env: {},
      homeDir: '/Users/tester',
    ...
    const initializedRoot = mod.resolveHostStateRoot({
      env: {},
      homeDir: '/Users/tester',
      nexusStateExists: false,
    });
