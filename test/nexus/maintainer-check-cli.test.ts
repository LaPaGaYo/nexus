...
describe('nexus-maintainer-check', () => {
  ...
  test('report-only mode reports state without writing maintainer-status artifacts', () => {
    const root = mkdtempSync(join(tmpdir(), 'nexus-maintainer-check-'));
    ...
    try {
      ...
      const result = Bun.spawnSync([SCRIPT, '--report-only'], {
        cwd: root,
        stdout: 'pipe',
        stderr: 'pipe',
        env: {
      ...
      expect(result.stdout.toString().trim()).toBe('ACTION_REQUIRED publish_release');
      expect(result.stderr.toString().trim()).toBe('');
      expect(readFileSync(reportPath, 'utf8')).toBe('existing-json\n');
      expect(readFileSync(markdownPath, 'utf8')).toBe('existing-markdown\n');
    } finally {
      rmSync(root, { recursive: true, force: true });
    }
  });

  test('no-write alias does not create maintainer-status artifacts', () => {
    const root = mkdtempSync(join(tmpdir(), 'nexus-maintainer-check-'));
