...
describeIfSelected('Review Dashboard Via Attribution', ['review-dashboard-via'], () => {
  ...
  beforeAll(() => {
    ...
    // nexus-review-read reads from ~/.nexus/projects/$SLUG/$BRANCH-reviews.jsonl
    // For the test, we'll write a mock nexus-review-read script that returns our test data
    ...
    // Write a mock nexus-review-read that returns our test data
    const mockBinDir = path.join(dashDir, '.mock-bin');
    fs.mkdirSync(mockBinDir, { recursive: true });
    fs.writeFileSync(path.join(mockBinDir, 'nexus-review-read'), [
      '#!/usr/bin/env bash',
    ...
    fs.chmodSync(path.join(mockBinDir, 'nexus-review-read'), 0o755);
...
Instead of running ~/.claude/skills/nexus/bin/nexus-review-read, run this mock: ${mockBinDir}/nexus-review-read
...
Skip the preamble, lake intro, and all other ship steps.
