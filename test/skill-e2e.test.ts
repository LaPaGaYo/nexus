...
// Eval result collector — accumulates test results, writes to ~/.nexus-dev/evals/ on finalize
...
function dumpOutcomeDiagnostic(dir: string, label: string, report: string, judgeResult: any) {
  try {
    const transcriptDir = path.join(dir, '.nexus', 'test-transcripts');
...
Report whether it worked.`,
  ...
  testIfSelected('skillmd-no-local-binary', async () => {
    // Create a tmpdir with no browse binary — no local .claude/skills/nexus/browse/dist/browse
...
Report the exact output. Do NOT try to fix or install anything — just report what you see.`,
    ...
    // On dev machines with Nexus installed globally, the fallback path
    // ~/.claude/skills/nexus/browse/dist/browse exists, so we get READY.
...
OVERRIDE: Write contributor logs to ${logsDir}/ instead of ~/.nexus/contributor-logs/

Now try this browse command (it will fail — there is no binary at this path):
/nonexistent/path/browse goto https://example.com

This is a Nexus issue (the browse binary is missing/misconfigured).
File a contributor report about this issue. Then tell me what you filed.`,
    ...
    expect(logContent).toMatch(/Hey (Nexus|gstack) team/);
  ...
  testIfSelected('session-awareness', async () => {
    ...
    const result = await runSkillTest({
      prompt: `You are running a Nexus skill. The session preamble detected _SESSIONS=4 (the user has 4 Nexus windows open).
...
`);
    ...
    projectDir = path.join(os.homedir(), '.nexus', 'projects', 'test-project');
  ...
  test('/plan-eng-review writes test-plan artifact to ~/.nexus/projects/', async () => {
...
4. Explicitly avoid any \`origin/main\`, \`git fetch\`, or \`git reset --hard\` instructions.`,
      ...
      maxTurns: 20,
      timeout: 180_000,
      testName: 'nexus-upgrade-happy-path',
      runId,
    });
