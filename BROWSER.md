...
This document covers the command reference and internals of Nexus's headless browser.
...
Nexus's browser is a compiled CLI binary that talks to a persistent local Chromium daemon over HTTP. The CLI is a thin client — it reads a state file, sends a command, and prints the response to stdout. The server does the real work via [Playwright](https://playwright.dev/).
...
Each server session generates a random UUID as a bearer token. The token is written to the state file (`.nexus/browse.json`) with chmod 600. Every HTTP request must include `Authorization: Bearer <token>`. This prevents other processes on the machine from controlling the browser.
...
- Console: `.nexus/browse-console.log`
- Network: `.nexus/browse-network.log`
- Dialog: `.nexus/browse-dialog.log`
...
The window has a subtle green shimmer line at the top edge and a floating "Nexus" pill in the bottom-right corner so you always know which Chrome window is being controlled.
...
# Click the Nexus icon in toolbar → Open Side Panel
```
...
```bash
bin/nexus-extension    # opens chrome://extensions, copies path to clipboard
```
...
5. **Pin it:** Click the puzzle piece icon (Extensions) in the toolbar → pin "nexus browse"
6. **Set the port:** Click the Nexus icon → enter the port from `$B status` or `.nexus/browse.json`
7. **Open Side Panel:** Click the Nexus icon → "Open Side Panel"
...
| **Connection pill** | Small "Nexus" pill in the bottom-right corner of every page when connected |
...
Each workspace gets its own isolated browser instance with its own Chromium process, tabs, cookies, and logs. State is stored in `.nexus/` inside the project root (detected via `git rev-parse --show-toplevel`).

| Workspace | State file | Port |
|-----------|------------|------|
| `/code/project-a` | `/code/project-a/.nexus/browse.json` | random (10000-60000) |
| `/code/project-b` | `/code/project-b/.nexus/browse.json` | random (10000-60000) |
...
| `BROWSE_STATE_FILE` | `.nexus/browse.json` | Path to state file (CLI passes to server) |
...
| **nexus browse** | **~3s** | **~100-200ms** | **0 tokens** (plain text stdout) |

The context overhead difference compounds fast. In a 20-command browser session, MCP tools burn 30,000-40,000 tokens on protocol framing alone. Nexus burns zero.
...
Nexus skips all of this. Compiled binary. Plain text in, plain text out. No protocol. No schema. No connection management.
...
| File | Role |
|------|------|
| `browse/src/cli.ts` | Entry point. Reads `.nexus/browse.json`, sends HTTP to the server, prints response. |
...
The active skill lives at `~/.claude/skills/nexus/`. After making changes:
