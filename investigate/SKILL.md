...
source <(~/.claude/skills/nexus/bin/gstack-repo-mode 2>/dev/null) || true
...
for _PF in $(find ~/.gstack/analytics -maxdepth 1 -name '.pending-*' 2>/dev/null); do
  if [ -f "$_PF" ]; then
    if [ "$_TEL" != "off" ] && [ -x "~/.claude/skills/nexus/bin/gstack-telemetry-log" ]; then
      ~/.claude/skills/nexus/bin/gstack-telemetry-log --event-type skill_run --skill _pending_finalize --outcome unknown --session-id "$_SESSION_ID" 2>/dev/null || true
    fi
...
# Learnings count
eval "$(~/.claude/skills/nexus/bin/gstack-slug 2>/dev/null)" 2>/dev/null || true
...
```bash
_CROSS_PROJ=$(~/.claude/skills/nexus/bin/gstack-config get cross_project_learnings 2>/dev/null || echo "unset")
echo "CROSS_PROJECT: $_CROSS_PROJ"
if [ "$_CROSS_PROJ" = "true" ]; then
  ~/.claude/skills/nexus/bin/gstack-learnings-search --limit 10 --cross-project 2>/dev/null || true
else
  ~/.claude/skills/nexus/bin/gstack-learnings-search --limit 10 2>/dev/null || true
fi
```
...
If A: run `~/.claude/skills/nexus/bin/gstack-config set cross_project_learnings true`
If B: run `~/.claude/skills/nexus/bin/gstack-config set cross_project_learnings false`
...
```bash
~/.claude/skills/nexus/bin/gstack-learnings-log '{"skill":"investigate","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
