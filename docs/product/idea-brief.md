...
## Source 13 + Incident (2026-05-18) — Finding E (pre-existing) + a git-stash incident I caused and recovered

### Finding E (pre-existing, NOT caused by the RFC) — repo-taxonomy hard-throws on an unclassified path

Writing the lifecycle RFC triggered `bun run repo:inventory` / `:check`, which fails: `repo-taxonomy.ts:652` `throw new Error("No repo taxonomy classification for path: .planning/current/frame/design-intent.json")`. Verified pre-existing: the same failure reproduces at commit `4c45299` (before the RFC). Root: earlier this session we wrote operator-attested Nexus framing artifacts to `.planning/current/frame/design-intent.json` on a branch whose `lib/nexus/io/repo-taxonomy.ts` has no rule for that path; the classifier **hard-throws** on any unanticipated path instead of bucketing it as `unclassified`. Same defect family as everything else this session (a guard/classifier that hard-fails on a legitimate-but-unanticipated input). NOT fixed here (scope discipline — separate bounded finding). The RFC commit `8ff78b2` is valid as-is; `repo-path-inventory.md` was intentionally NOT regenerated because the regenerator is pre-existing-broken on this branch, unrelated to the RFC.

### Incident (operator-caused, recovered, disclosed) — careless git stash pop on an unrelated stash

While diagnosing Finding E I ran `git stash` / `git stash pop` from this worktree without verifying stash contents. A repo-global pre-existing stash `stash@{0}: WIP on sp1-signal-architecture: 640ff2d` (another branch/context's uncommitted work — a rebuilt `bin/nexus-global-discover` binary) was popped, churned, and dropped from the stash list. This is precisely the session's invariant bug class (treat a destructive op as safe without verifying), this time committed by the operator on another context's uncommitted work. Recovered: the dangling stash commit `db7797c7` was located via `git fsck --unreachable` and precisely re-stored with `git stash store` under its original message; `git stash list` and `git stash show` confirm it is back identical to the pre-incident state. No data lost. Disclosed here rather than buried — which is the RFC's own thesis (gates with teeth must catch the operator's own errors, not just others').
