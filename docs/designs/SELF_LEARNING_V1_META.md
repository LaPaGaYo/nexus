...
|---|---|---|---|---|
| **SP1** | **Signal Architecture** ✅ **COMPLETE** | Evolve `learnings.jsonl` schema + cross-skill capture conventions; introduce `learning-candidates.json` provisional layer. Shipped: schema v2 (19 fields incl. `subject_skill`, `subject_stage`, `evidence_type`, `cluster_id`, `supersedes`, `supersedes_reason`, `derived_from`, `last_applied_at`, `mirror`); `evidence_strength` resolved as **derived** (`computeStrength(evidence_type, source, confidence)` — single SP6/SP2 source of truth, no stored field); ULID ids + lazy v1→v2 read normalization; direct template (`/investigate` `/retro` `/simplify` `/cso`) + chain template (`/build` `/review` `/qa` → candidates → `/closeout` promotion → opt-in mirror). See `docs/designs/SELF_LEARNING_V1_SP1.md` + `_SP1_PLAN.md`. | none | ~~now~~ **landed** |
...
Status: **SP1 complete** (landed 2026-05-17) + **SP1-followup** (shared decay/reader primitives, merged `891055c`) + **SP6 v1 landed 2026-05-18** (branch `sp6-learning-context`: read-only `LearningContextResolver` wired to the completion-advisor via `stageAwareAdvisor` → `recommended_skills`; all 5 ACs proven by construction; the other 3 consumers + caching deferred to SP6.1). **SP4 + SP6.1 + SP2 are the open candidates** — SP2's trigger (`SP6 ships to production`, per glaocon V1 feedback) goes live once SP6 v1 is in production use.
...
| 2026-05-17 | SP1 implementation landed (branch `sp1-signal-architecture`, 29 commits) | All 22 SP1 plan tasks shipped via subagent-driven-development. Resolved Issue-2 `evidence_strength` as **derived** (no stored field; `computeStrength` is the single SP6/SP2 source of truth). 4 review-driven fixes (Concern A v2-passthrough, Concern B validator guards, T14 capture breadcrumb, T21 v1-merge data-loss). Concern E surfaced + fixed: `LEARNING_SOURCES` drift (`contracts/types` 4→8 canonical, `schema.ts` re-exports) — also fixed a latent pre-SP1 auditor-candidate-drop bug. | §4 (SP1 → ✅ COMPLETE; status line: SP4+SP6 unblocked). SP6 is now the recommended next sub-project (its `SP1 lands` trigger is met; glaocon V1 feedback paired it with SP1 and deferred SP2 behind it). |
| 2026-05-18 | SP1-followup + SP6 v1 landed | SP1-followup (PR #162, `891055c`): extracted shared `computeEffectiveConfidence` (decay) + 4 fail-soft readers, barreled for SP6 — closed the 2 SP6-readiness contract gaps with one accepted anti-drift convergence (Task E archive-walk now uses the validated canonical reader). SP6 v1 (branch `sp6-learning-context`, 11 tasks via subagent-driven-development): read-only `LearningContextResolver` (transparent weighted-linear-sum ranking, score-floor + dual cap, bounded additive boost, `learning-context.json` explainability) wired to the completion-advisor at `writer.ts:48` via `stageAwareAdvisor` → `recommended_skills`. All 5 ACs proven by construction (AC#5 confirmed via cwd directory-scan: exactly one file written). Planning-found correction: boost retargeted from the score-discarding `rankInstalledSkillsForAdvisor` to the score-carrying `stageAwareAdvisor`. Behavior-preservation gate held (advisor baseline byte-identical; multi-skill empty-store order-equivalence proven). | §4 (status line: SP1-followup + SP6 v1 landed; SP4 + SP6.1 + SP2 now open; SP2 trigger goes live on SP6 production use). §6 unchanged (SP2-after-SP6 ordering already recorded). §9 (this row). |
