...
```bash
REMOTE_SLUG=$(browse/bin/remote-slug 2>/dev/null || ~/.claude/skills/nexus/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
PROJECT_HISTORY="$HOME/.nexus/projects/$REMOTE_SLUG/greptile-history.md"
```
...
```bash
REMOTE_SLUG=$(browse/bin/remote-slug 2>/dev/null || ~/.claude/skills/nexus/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
mkdir -p "$HOME/.nexus/projects/$REMOTE_SLUG"
mkdir -p ~/.nexus
```

Append one line per triage outcome to **both** files (per-project for suppressions, global for retro):
- `~/.nexus/projects/$REMOTE_SLUG/greptile-history.md` (per-project)
- `~/.nexus/greptile-history.md` (global aggregate)
