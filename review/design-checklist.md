...
**Trigger:** Only run this checklist if the diff touches frontend files. Use `nexus-diff-scope` to detect:

```bash
source <(~/.claude/skills/nexus/bin/nexus-diff-scope <base> 2>/dev/null)
```
