...
**Local:** \`.nexus/design-reports/design-audit-{domain}-{YYYY-MM-DD}.md\`

**Project-scoped:**
\`\`\`bash
eval "$(${ctx.paths.binDir}/nexus-slug 2>/dev/null)" && mkdir -p ~/.nexus/projects/$SLUG
\`\`\`
Write to: \`~/.nexus/projects/{slug}/{user}-{branch}-design-audit-{datetime}.md\`
...
**Log the result:**
\`\`\`bash
${ctx.paths.binDir}/nexus-review-log '{"skill":"design-outside-voices","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","commit":"'"$(git rev-parse --short HEAD)"'"}'
\`\`\`
