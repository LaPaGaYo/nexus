 ...
 *
 * Review logs are stored locally at ~/.nexus/reviews/review-log.jsonl.
...
3. Append metrics:
\`\`\`bash
mkdir -p ~/.nexus/review-metrics
echo '{"skill":"${ctx.skillName}","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","iterations":ITERATIONS,"issues_found":FOUND,"issues_fixed":FIXED,"remaining":REMAINING,"quality_score":SCORE}' >> ~/.nexus/review-metrics/spec-review.jsonl 2>/dev/null || true
\`\`\`
...
SLUG=$(${ctx.paths.skillRoot}/browse/bin/remote-slug 2>/dev/null || ${ctx.paths.skillRoot}/runtimes/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
BRANCH=$(git rev-parse --abbrev-ref HEAD 2>/dev/null | tr '/' '-' || echo 'no-branch')
DESIGN=$(ls -t ~/.nexus/projects/$SLUG/*-$BRANCH-design-*.md 2>/dev/null | head -1)
[ -z "$DESIGN" ] && DESIGN=$(ls -t ~/.nexus/projects/$SLUG/*-design-*.md 2>/dev/null | head -1)
...
\`\`\`bash
CODEX_PROMPT_FILE=$(mktemp /tmp/nexus-codex-oh-XXXXXXXX.txt)
\`\`\`
...
\`\`\`bash
${ctx.paths.binDir}/nexus-review-log '{"skill":"adversarial-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","tier":"always","gate":"GATE","commit":"'"$(git rev-parse --short HEAD)"'"}'
\`\`\`
...
**Persist the result:**
\`\`\`bash
${ctx.paths.binDir}/nexus-review-log '{"skill":"codex-plan-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","commit":"'"$(git rev-parse --short HEAD)"'"}'
\`\`\`
...
# Compute project slug for ~/.nexus/projects/ lookup
_PLAN_SLUG=$(git remote get-url origin 2>/dev/null | sed 's|.*[:/]\\([^/]*/[^/]*\\)\\.git$|\\1|;s|.*[:/]\\([^/]*/[^/]*\\)$|\\1|' | tr '/' '-' | tr -cd 'a-zA-Z0-9._-') || true
_PLAN_SLUG="\${_PLAN_SLUG:-$(basename "$PWD" | tr -cd 'a-zA-Z0-9._-')}"
# Search common plan file locations (project plans first, then personal/local)
for PLAN_DIR in "$HOME/.nexus/projects/$_PLAN_SLUG" "$HOME/.claude/plans" "$HOME/.codex/plans" ".nexus/plans"; do
  [ -d "$PLAN_DIR" ] || continue
...
- Review report sections (\`## NEXUS REVIEW REPORT\`)
...
\`\`\`bash
${ctx.paths.binDir}/nexus-learnings-log '{
  "type": "pitfall",
