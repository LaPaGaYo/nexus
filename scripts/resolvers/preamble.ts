...
if [ -n "$_LOCAL_PROVIDER_CANDIDATE" ]; then
  ${ctx.paths.binDir}/nexus-config set primary_provider "$_LOCAL_PROVIDER_CANDIDATE"
fi
if [ -n "$_LOCAL_PROVIDER_TOPOLOGY" ]; then
  ${ctx.paths.binDir}/nexus-config set provider_topology "$_LOCAL_PROVIDER_TOPOLOGY"
fi
\`\`\`
...
If B: do not change Nexus config. Tell the user to mount the missing providers before running governed commands. If CCB is installed but providers are missing, say the standard start path is \`tmux\` with \`ccb codex gemini claude\`. If CCB is not installed, say they need to install or restore CCB first.`
    : `If \`EXECUTION_MODE=governed_ccb\` and \`CURRENT_SESSION_READY\` is \`no\` and \`LOCAL_PROVIDER_READY\` is \`yes\`, present a plain numbered choice and wait for the user response before moving into lifecycle work:

1. Switch this host to local_provider (recommended if the user wants to work now)
2. Keep governed_ccb and mount the missing CCB providers
...
If the user chooses 2: do not change Nexus config. Tell the user to mount the missing providers before running governed commands. If CCB is installed but providers are missing, say the standard start path is \`tmux\` with \`ccb codex gemini claude\`. If CCB is not installed, say they need to install or restore CCB first.`}
...
\`\`\`
Then explain that the current session can continue with \`local_provider\`, and if \`PROVIDER_TOPOLOGY\` is empty the default local topology comes from \`${ctx.paths.binDir}/nexus-config effective-execution\` (Claude Code hosts default Claude to \`subagents\`; direct terminals default to \`single_agent\`).
