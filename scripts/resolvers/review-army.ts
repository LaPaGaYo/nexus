 ...
 * 1. Detect stack and scope (via nexus-diff-scope)
...
\`\`\`bash
source <(${ctx.paths.binDir}/nexus-diff-scope <base> 2>/dev/null) || true
...
\`\`\`bash
${ctx.paths.binDir}/nexus-learnings-search --type pitfall --query "{specialist domain}" --limit 5 2>/dev/null || true
\`\`\`
