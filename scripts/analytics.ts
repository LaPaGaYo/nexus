#!/usr/bin/env bun
/**
 * analytics — CLI for viewing Nexus skill usage statistics.
 *
 * Reads ~/.nexus/analytics/skill-usage.jsonl and displays:
...
const ANALYTICS_FILE = path.join(os.homedir(), '.nexus', 'analytics', 'skill-usage.jsonl');
...
export function formatReport(events: AnalyticsEvent[], period: string = 'all'): string {
  ...
  const lines: string[] = [];
  lines.push('Nexus skill usage analytics');
