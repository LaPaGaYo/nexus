/**
 * nexus browse server — persistent Chromium daemon
 *
 * Architecture:
 ...
 *
 * State:
 *   State file: <project-root>/.nexus/browse.json (set via BROWSE_STATE_FILE env)
 *   Log files:  <project-root>/.nexus/browse-{console,network,dialog}.log
...
function generateHelpText(): string {
  ...
  const lines = ['nexus browse — headless browser for AI agents', '', 'Commands:'];
...
const SESSIONS_DIR = path.join(process.env.HOME || '/tmp', '.nexus', 'sidebar-sessions');
...
function createWorktree(sessionId: string): string | null {
  try {
    ...
    const worktreeDir = path.join(process.env.HOME || '/tmp', '.nexus', 'worktrees', sessionId.slice(0, 8));
