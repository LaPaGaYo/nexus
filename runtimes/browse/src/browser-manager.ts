...
export class BrowserManager {
  ...
  async launch() {
    ...
    this.browser.on('disconnected', () => {
      ...
      console.error('[browse] Console/network logs flushed to .nexus/browse-*.log');
      process.exit(1);
    });
  ...
  // ─── Headed Mode ─────────────────────────────────────────────
  /**
   * Launch Playwright's bundled Chromium in headed mode with the Nexus
  ...
  async launchHeaded(authToken?: string): Promise<void> {
    ...
    const indicatorScript = () => {
      const injectIndicator = () => {
        if (document.getElementById('nexus-ctrl')) return;

        const topLine = document.createElement('div');
        topLine.id = 'nexus-ctrl';
        ...
        style.textContent = `
          ...
          @media (prefers-reduced-motion: reduce) {
            #nexus-ctrl { animation: none !important; }
          }
        `;
